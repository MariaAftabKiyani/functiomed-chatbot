...
// Language-specific messages for UI elements
const MESSAGES = {
    EN: {
    ...
    DE: {
        initialGreeting: "Hallo! 👋 Ich bin FUNIA, Ihr freundlicher Assistent bei functiomed. Ich bin hier, um Ihnen bei allem zu helfen, was Sie brauchen - sei es Informationen über unsere Dienstleistungen, Ärzte oder die Beantwortung Ihrer Fragen. Womit kann ich Ihnen heute helfen?",
    ...
    FR: {
        initialGreeting: "Bonjour! 👋 Je suis FUNIA, votre assistant amical chez functiomed. Je suis ici pour vous aider avec tout ce dont vous avez besoin - que ce soit pour trouver des informations sur nos services, nos médecins ou pour répondre à vos questions. Comment puis-je vous aider aujourd'hui?",
...
// Helper function to find matching FAQ based on exact question match or FAQ ID
function findMatchingFAQ(userQuery) {
    if (!userQuery) return null;

    // Normalize user query for comparison (trim, lowercase, remove all special characters and numbers, collapse spaces)
    const normalizedQuery = userQuery.trim().toLowerCase().replace(/[~!@#$%^&*()\-+=\[\]{}0-9`;':"\\|<>?,./]/g, '').replace(/\s+/g, ' ').trim();

    // First, check if the query matches an FAQ ID directly
    if (faqCache[normalizedQuery]) {
        return faqCache[normalizedQuery];
    }

    // Also try with spaces converted to underscores (for FAQ IDs like "osteopathy_suitability")
    const normalizedQueryWithUnderscores = normalizedQuery.replace(/\s+/g, '_');
    if (faqCache[normalizedQueryWithUnderscores]) {
        return faqCache[normalizedQueryWithUnderscores];
    }

    // Also check by removing underscores from user query and comparing with FAQ IDs without underscores
    // This handles cases like "osteopathysuitability" matching "osteopathy_suitability"
    const queryWithoutUnderscores = normalizedQuery.replace(/_/g, '');
    for (const faqId in faqCache) {
        const faqIdWithoutUnderscores = faqId.replace(/_/g, '');
        if (queryWithoutUnderscores === faqIdWithoutUnderscores) {
            return faqCache[faqId];
        }
    }

    // If not an ID, search through all FAQs for question match
    for (const faqId in faqCache) {
        const faq = faqCache[faqId];

        // Support both old format (question: {}) and new format (questions: {})
        const questionData = faq.questions || faq.question;

        if (questionData && questionData[currentLanguage]) {
            // Handle array of questions (new format)
            if (Array.isArray(questionData[currentLanguage])) {
                for (const question of questionData[currentLanguage]) {
                    // Normalize FAQ question (lowercase, remove all special characters and numbers, collapse spaces)
                    const faqQuestion = question.toLowerCase().replace(/[~!@#$%^&*()\-+=\[\]{}0-9`;':"\\|<>?,./]/g, '').replace(/\s+/g, ' ').trim();
                ...
                // Handle single question (old format)
                const faqQuestion = questionData[currentLanguage].toLowerCase().replace(/[~!@#$%^&*()\-+=\[\]{}0-9`;':"\\|<>?,./]/g, '').replace(/\s+/g, ' ').trim();
...
function updateFAQButtonTexts() {
    ...
    faqButtons.forEach(button => {
        ...
        if (faq) {
            ...
            if (questionData && questionData[currentLanguage]) {
                // If it's an array, use the first question
                const buttonText = Array.isArray(questionData[currentLanguage])
                    ? questionData[currentLanguage][0]
                    : questionData[currentLanguage];
...
async function handleFAQClick(faqId) {
    ...
    // Support both old format (question: {}) and new format (questions: {})
    const questionData = faq.questions || faq.question;
    const question = Array.isArray(questionData[currentLanguage])
        ? questionData[currentLanguage][0]  // Use first question from array
        : questionData[currentLanguage];
...
async function changeLanguage(lang) {
    if (!lang || !['EN', 'DE', 'FR'].includes(lang)) {
...
function updateUILanguage() {
    ...
    if (!messages) {
        console.error('Messages not found for language:', currentLanguage);
        return;
    }
    ...
    if (chatInput) chatInput.placeholder = messages.placeholder;

    // Update initial greeting message if it exists
    const initialMessage = document.querySelector('.message.bot .message-content');
    if (initialMessage && conversationHistory.length === 0) {
        initialMessage.textContent = messages.initialGreeting;
    }
}
...
function init() {
    ...
    if (!chatWidget || !chatButton) {
        console.error('Chat widget elements not found. Make sure the HTML is loaded.');
        return;
    }
    ...
    // Set active language button (EN by default)
    languageButtons.forEach(btn => {
    ...
    // Update initial message and show it
    const initialMessage = document.getElementById('initialMessage');
    if (initialMessage) {
        const messages = MESSAGES[currentLanguage];
        const messageContent = initialMessage.querySelector('.message-content');
        const messageTime = initialMessage.querySelector('.message-time');

        if (messageContent) {
            messageContent.innerHTML = markdownToHtml(messages.initialGreeting);
        }
        if (messageTime) {
            messageTime.textContent = getCurrentTime();
        }

        // Remove the initial-hidden class to show the message
